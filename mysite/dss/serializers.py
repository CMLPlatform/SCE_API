    ...
    def validate(self, data):
        ...
        if t not in ["min", "max"]:
            try:
                ...
                raise ValidationError(f"Target not recognized: {t}")
        return data
...
class ExperimentComparisonSerializer(serializers.ListSerializer):
    ...
    def validate(self, data):
        ...
        if len(ids) != len(data):
            raise ValidationError("Duplicate experiment IDs found")
    ...
    def validate(self, data):
        ...
        for dir in data["directions"].values():
            if not isinstance(dir, (str, int, float)):
                raise ValidationError(f"Wrong data in directions: {dir}")
            elif isinstance(dir, str) and dir not in ["min", "max"]:
                raise ValidationError(f"Expected 'min' or 'max', received: {dir}")
        ...
        if data["groups"]:
            ...
            for field in ["group_weights", "local_weights", "group_ranks", "local_ranks"]:
                if dic := data.get(field):
                    ...
                    if missing:
                        raise ValidationError(
                            {field: f"Unrecognized groups: {missing}"}
                        )
        ...
        if data["weight_mode"] == "group":
            if not data["groups"]:
                ValidationError("For grouped weighting, specify 'groups'")
        elif data["weight_mode"] == "hierarchical":
            if (not data["group_weights"]) or (data["groups"]):
                ValidationError(
                    "Hierarchical weighting requires both 'groups' and 'group_weights'"
                )
